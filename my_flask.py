...
def file_upload():
    if 'file_zadatele' in request.files:
        ...
        if file_zadatele.filename != '':
            ...
            # flash(F'Dobře ty! CSV s názvem {new_filename_zadatele} se teď stahuje do tvé Downloads složky')
            return send_file(new_file_path, as_attachment=True)
        else:
            flash('Nemáš vybranej soubor!')
        ...
        if file_projekty.filename != '':
            ...
            # flash(F'Dobře ty! CSV s názvem {new_filename_projekty} se teď stahuje do tvé Downloads složky')
            return send_file(new_file_path, as_attachment=True)
        else:
            flash('Nemáš vybranej soubor!')
